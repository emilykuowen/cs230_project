...
def train(output_folder, batch_size, max_epochs, epoch_length):    
    ...
    #train_folder = './medleydb_acoustic_guitar/train' # TODO should already have scraper mixed tracks
    train_folder = '~/data/medleydb_' + 'acoustic_guitar'
    #train_folder = fg_folder
    #bg_folder = '~/data/bg_' + 'acoustic_guitar'
    #val_folder = "~/.nussl/tutorial/valid"
    #val_folder = './medleydb_acoustic_guitar/val' 
    #ms_folder = './mix_source_folder' + '_' + 'acoustic_guitar'
    # create MixSourceFolder-style dataset 
    #msf = nussl.datasets.MixSourceFolder(ms_folder)
    # TODO ^ processed before with scraper to generate items in train, val, and test?
    ...
    train_data = get_medley_data.on_the_fly(stft_params, transform=tfm, 
...
def evaluate(separator, output_path):
    ...
    for i in range(len(test_dataset)):
        ...
        estimates = {
            'acoustic guitar': estimates[0],
            'other': item['mix'] - estimates[0]
        }

        # write audio output to wav
        estimates['acoustic guitar'].write_audio_to_file(output_path + filename + '_acoustic_guitar.wav')
        estimates['other'].write_audio_to_file(output_path + filename + '_other.wav')
...
def plot_validation_loss(filepath, output_path):
    ...
    plt.title('Validation Loss of Acoustic Guitar Separator Model')
...
if __name__ == "__main__":
    ...
    output_path = 'acoustic_guitar_output/'
    output_folder = Path(output_path).absolute()
